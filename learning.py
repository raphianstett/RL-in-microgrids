...
class Baseline:
    def find_baseline_policy(data, mdp):
        ...
        diffs = []
        battery = []
        current_state = State(data["Consumption"][0], data["Production"][0], 2,data["Production"][1], data["Time"][0], mdp)
        
        for i in range(1,len(data["Consumption"])):
            
            if current_state.p - current_state.c >= 1000 and current_state.battery + mdp.step_high <= mdp.max_battery:
                action = "charge_high"
                # print("charge_high")
            elif current_state.p - current_state.c >= mdp.charge_low and current_state.battery + mdp.step_low_charge <= mdp.max_battery:
                action = "charge_low"
                # print("charge_low")
            elif (current_state.c - current_state.p) >= mdp.discharge_low and current_state.battery - mdp.step_high >= 0.0:
                action = "discharge_high"
                # print("discharge_high")
            elif (current_state.c - current_state.p) > 0.0 and current_state.battery - mdp.step_low_discharge >= 0.0:
            ...
            battery.append(current_state.battery)
            diffs.append((current_state.c - current_state.p) >= mdp.max_discharge and current_state.battery - mdp.step_high >= 0)
